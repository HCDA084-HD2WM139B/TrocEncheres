...
/**
 * Classe de la DAL implémentant l'interface UtilisateurDAO permettant de
 * réaliser les requetes (CRUD) à l'entité utilisateur d'une Base de données SQL
 * Server.
 * 
 * @author Groupe 3
 *
...
public class UtilisateurDAOImpl implements UtilisateurDAO {
	...
	/**
	 * Méthode permettant d'ajouter un utilisateur, passé en paramètre, dans la base
	 * de données sur la table UTILISATEUR.
	 * 
	 * @param utilisateur
	 *            (Utilisateur) L'utilisateur à enregistrer.
	 */
	@Override
	public Utilisateur insertUtilisateur(Utilisateur utilisateur) throws BusinessException {
		...
		try (Connection cnx = ConnectionProvider.getConnection();
				PreparedStatement psmt = cnx.prepareStatement(INSERT_UTILISATEUR,
		...
		if (utilisateur.getNoUtilisateur() <= 100) {
			try (Connection cnx = ConnectionProvider.getConnection();
					PreparedStatement psmt = cnx.prepareStatement(INSERT_CREDIT,
							PreparedStatement.RETURN_GENERATED_KEYS);) {
				// On prépare la requête SQL pour insérer un utilisateur et on récupère l'ID
				// généré par l'insertion
				psmt.setInt(1, 100);
	...
	/**
	 * Méthode retournant un utilisateur en fonction d'un identifiant, d'un mot de
	 * passe, et du type d'identifiant passés en paramètre.
	 * 
	 * @param pTypeRequete
	 *            (String) Le type d'identifiant correspondant au nom exact de la
	 *            colonne en table Utilisateur de la Base de données.
	 * @param pIdentifiant
	 *            (String) Le login valide saisie par l'utilisateur.
	 * @param pMotDePasse
	 *            (String) Le mot de passe valide saisie par l'utilisateur.
	 * @return (Utilisateur) un utilisateur si les identifiants existent en base de
	 *         données, sinon null.
	 */
	@Override
	public Utilisateur selectByLogin(String pTypeRequete, String pIdentifiant, String pMotDePasse)
			throws BusinessException {
		Utilisateur utilisateurTrouve = null;
		// Vérification des paramètres (le type doit etre correct !)
		if (pIdentifiant == null || pTypeRequete == null || pMotDePasse == null
				|| (!pTypeRequete.equals(COLONNE_EMAIL) && !pTypeRequete.equals(COLONNE_PSEUDO))) {
			throw new BusinessException();
		}
		...
		try (Connection cnx = ConnectionProvider.getConnection(); PreparedStatement psmt = cnx.prepareStatement(sql)) {
			...
			// Si la requete retourne une ligne (on considère une seule réponse car l'email
			// et le pseudo sont uniques en BDD)
			if (rs.next()) {
				// on hydrate notre nouvelle utilisateur
				utilisateurTrouve = new Utilisateur(rs.getInt("no_utilisateur"), rs.getString("pseudo"),
	...
	/**
	 * Méthode qui recherche un utilisateur avec l'ID ET le Pseudo correspondant ou
	 * non
	 * 
	 * @param pPseudo
	 *            (String) Le pseudo de l'utilisateur.
	 * @param pNo_utilisateur
	 *            (Integer) Le numéro de l'utilisateur.
	...
	/**
	 * Méthode sélectionnant toute la liste des pseudos des utilisateurs enregistrés
	 * en base de données afin de vérifier si l'utilisateur à inscrire ne choisisse
	 * pas un pseudo déjà existant.
	...
	/**
	 * Méthode sélectionnant toute la liste des emails des utilisateurs enregistrés
	 * en base de données afin de vérifier si l'utilisateur à inscrire ne choisisse
	 * pas un email déjà existant.
	 * @param pEmail : E-mail proposé par l'utilisateur
	 */
	@Override
	...
	/**
	 * Méthode exécutant la procédure stockée servant à la suppression et
	 * l'archivage d'un utilisateur. La procédure stockée va enregistrer une copie
	 * de l'utilisateur dans la table "Archives_utilisateurs" ; puis annuler les
	 * enchères concernant l'utilisateur (acheteur) ; puis supprimer l'utilisateur
	 * (vendeur) avec tous ses articles qui lui sont affectés mais aussi les
	 * enchères affectés à ces mêmes articles ; et enfin supprimer les enchères
	 * préalablement annulées le concernant.
	 * 
	 * @param (int)
	 *            id ; le numero d'utilisateur en base de données.
	 * @return (boolean) True si la procédure stockée execute correctement
	 *         l'archivage et la suppression, sinon False.
	...
	public boolean deleteUserById(int id) throws BusinessException {
		...
		try (Connection cnx = ConnectionProvider.getConnection(); CallableStatement csmt = cnx.prepareCall(sql)) {
			...
			// Si la procédure stockée nous renvoi 1, c'est qu'elle a réussi la suppression
			// et l'archivage !
			if (reponse == 1) {
				result = true;
				// transaction (fin)
